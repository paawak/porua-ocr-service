...
public class EntityClassUtil {
    ...
    private static final String DYNAMIC_ENTITY_PACKAGE = Book.class.getPackageName() + DYNAMIC_PACKAGE_SUFFIX;
    private static final String DYNAMIC_REPOSITORY_PACKAGE =
	    BookRepository.class.getPackageName() + DYNAMIC_PACKAGE_SUFFIX;
	...
	String baseName = baseNameBuilder
		.replace(0, 1, Character.toString(baseNameBuilder.charAt(0)).toUpperCase()).toString();

	return new EntityClassDetails(DYNAMIC_ENTITY_PACKAGE + baseName + OCR_WORD_ENTITY_SUFFIX,
		DYNAMIC_ENTITY_PACKAGE + baseName + CORRECTED_WORD_ENTITY_SUFFIX,
		DYNAMIC_REPOSITORY_PACKAGE + baseName + OCR_WORD_ENTITY_REPOSITORY_SUFFIX,
		DYNAMIC_REPOSITORY_PACKAGE + baseName + CORRECTED_WORD_ENTITY_REPOSITORY_SUFFIX);
    }
